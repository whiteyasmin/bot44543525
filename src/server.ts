...
<main>
  ...
  <section><h2>常用参数</h2><form id="quickSettings" class="quick-settings"></form><button id="saveQuick">保存常用参数</button></section>
...
const labels = {
 ...
 maxEntryPrice:'最高买入价', maxPositionUsdc:'单次最大 USDC', maxShares:'最大份额', depthUsageRatio:'使用盘口比例', minOrderUsdc:'最小订单 USDC',
...
const quickFields = [
 ['paperBalance','number'],['maxPositionUsdc','number'],['minBtcMoveBps','number'],['minBtcVelocityBps','number'],
 ['maxEntryPrice','number'],['takeProfitCents','number'],['stopLossCents','number'],['panicHedgeEnabled','checkbox']
];
const advancedFields = [
 ...
 ['maxEntryPrice','number'],['maxPositionUsdc','number'],['maxShares','number'],['depthUsageRatio','number'],['minOrderUsdc','number'],
...
async function loadDashboard(){const d=await (await fetch('/api/dashboard')).json();const s=d.state, cfg=d.settings;status.textContent=s.lastError?'错误':(cfg.botEnabled?'策略运行':'策略暂停');statusCard.className=cfg.botEnabled?'on':'off';action.textContent=s.lastError||actionText(s.lastAction);btc.textContent=s.btc?fmt(s.btc.price,2):'-';move.textContent='动量 '+fmt(s.moveBps,2)+' bps / 速度 '+fmt(s.velocityBps,2);market.textContent=s.currentMarket?s.currentMarket.slug.replace('btc-updown-5m-',''):'-';sec.textContent='第 '+s.secondInBucket+' 秒';bal.textContent=fmt(s.paperBalance,2)+' USDC';pnl.textContent='PnL '+fmt(s.realizedPnl,2);up.innerHTML=bookHtml(s.upBook);down.innerHTML=bookHtml(s.downBook);pos.innerHTML=posHtml(s.position);trades.innerHTML=tradeRows(d.recentTrades);events.innerHTML=eventRows(d.recentEvents)}
