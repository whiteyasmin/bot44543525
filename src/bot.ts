...
const DUAL_SIDE_MIN_ASK = 0.20;            // 挂单价下限: 过滤历史表现较差的 <$0.20 低价票
const DUAL_SIDE_MAX_ASK = 0.35;            // 挂单价上限 (≤0.35保证EV+$0.15/share@50%胜率)

const DUAL_SIDE_MIN_DRIFT = 0.04;          // 价格偏移>此值才重挂 (降低更新频率)
const DUAL_SIDE_MIN_VOL = 0.0010;          // 5分钟BTC波动率下限 (0.10%), 低于此视为微行情不挂单
const REACTIVE_MISPRICING_ENABLED = false; // Disable active dump-chasing entries; keep maker pre-orders only.
...
export interface HedgeHistoryEntry {
  ...
  entryAskSum?: number;     // 入场时 UP ask + DOWN ask
  entryVolatility5m?: number; // 入场时 5分钟BTC波动率
}
...
export class Hedge15mEngine {
  ...
  private leg1EntryAskSum = 0;
  private leg1EntryVolatility5m = 0;
  ...
  private resetRoundState(): void {
    ...
    this.leg1EdgeTier = "--";
    this.leg1EntryAskSum = 0;
    this.leg1EntryVolatility5m = 0;
      ...
      try {
        ...
        if (this.hedgeState === "watching") {
          ...
          if (this.upAsk > 0 && this.downAsk > 0) {
            ...
            if (REACTIVE_MISPRICING_ENABLED && dumpBaseline) {
  ...
  private async transitionPreOrderToLeg1(
    ...
    this.leg1EntryAskSum = this.upAsk > 0 && this.downAsk > 0 ? this.upAsk + this.downAsk : observedSum;
    this.leg1EntryVolatility5m = getRecentVolatility(300);
  ...
  private async openLeg1Position(
    ...
    try {
      ...
      this.leg1EntryAskSum = this.upAsk > 0 && this.downAsk > 0 ? this.upAsk + this.downAsk : 0;
      this.leg1EntryVolatility5m = getRecentVolatility(300);
      this.leg1EntryTrendBias = this.currentTrendBias;
      this.leg1EntrySecondsLeft = Math.floor(this.secondsLeft);
  ...
  private async settleHedge(): Promise<void> {
    ...
    this.history.push({
      ...
      entryAskSum: this.leg1EntryAskSum,
      entryVolatility5m: this.leg1EntryVolatility5m,
    });
